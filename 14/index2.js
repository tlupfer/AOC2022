...
const yMin = 0;
const xAxis = xMax - xMin + 1001;
const yAxis = yMax - yMin + 3;
const grid = [...Array(yAxis).keys()].map(() => [...Array(xAxis).keys()].map(() => '.'));
grid[grid.length - 1] = [...Array(xAxis).keys()].map(() => '#');

lines.forEach((line) => {
  const points = line.match(/\d+,\d+/g).map(p => p.split(',').map(Number));
  for (let i = 0; i < points.length; i++) {
    const [x, y] = points[i];
    const xIndex = x - xMin + 500;
    grid[y][xIndex] = '#';
    if (points[i - 1]) {
      const [lastX, lastY] = points[i - 1];
      const lastXindex = lastX - xMin + 500;
